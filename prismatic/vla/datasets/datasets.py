...
class RLDSBatchTransform:
    ...
    # Mirror-VLA related
    mask_cam: str = 'None'  # options: 'None', 'wrist', 'third'
    history_frames: int = 0 # Deprecated: history conditioning disabled
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # History is disabled: always use the full trajectory and current_action at t=0
        R = 1
        history_end = 0
        dataset_name, current_action = rlds_batch["dataset_name"], rlds_batch["action"][0]
        img_np = rlds_batch["observation"]["image_primary"] # [T, H, W, 3]
        imgs = [Image.fromarray(img) for img in img_np]
        ...
        # Get future action chunk (no leakage: history is strictly before history_end)
        future_actions = rlds_batch["action"][history_end:]
        ...
        pixel_values_all = torch.stack([self.image_transform(img) for img in imgs],dim=0) # [T, H, W, 3]
        actions_all = torch.stack([torch.tensor(action) for action in actions],dim=0) # [T, 7]
        
        if self.enable_four_quadrant_input:
            aug_mode = random.randint(0, 3)
            if aug_mode == 1:
                pixel_values_all = torch.flip(pixel_values_all, dims=[3])
            elif aug_mode == 2:
                actions_all[:,[1,3,5]] *= -1
            elif aug_mode == 3:
                pixel_values_all = torch.flip(pixel_values_all, dims=[3])
        ...
        # History is disabled: only keep final image and full action sequence
        pixel_values_history = None
        actions_history = None
        pixel_values = pixel_values_all[-1]
        actions = actions_all
        ...
        # Mask camera views if specified (for Mirror-VLA experiments)
        if self.mask_cam != 'None':
            if self.mask_cam == 'wrist':
                return_dict['pixel_values_wrist'] = torch.zeros_like(return_dict['pixel_values_wrist'])
            elif self.mask_cam == 'third':
                return_dict['pixel_values'] = torch.zeros_like(return_dict['pixel_values'])
...
class RLDSDataset(IterableDataset):
    def __init__(
        ...
        history_compression_rate: int = 1,
    ) -> None:
        ...
        rlds_config = dict(
            traj_transform_kwargs=dict(
                # History disabled: always use single-step window
                window_size               = 1,
