...
class L1RegressionActionHead(nn.Module):
    ...
    def __init__(
        ...
        use_aug_mode_condition=False,
    ):
        super().__init__()
        self.num_task_tokens = num_task_tokens
        self.action_dim = action_dim
        self.hidden_dim = hidden_dim
        self.use_history_version = use_history_version
        self.use_aug_mode_condition = use_aug_mode_condition
        
        if self.use_aug_mode_condition:
            self.aug_mode_proj = nn.Embedding(4, hidden_dim)
        else:
            self.aug_mode_proj = None
    ...
    def predict_action(
            ...
            phase="Inference",
            actions_hidden_states_history=None,
        ...
        task_hidden_states = actions_hidden_states[:, :, :self.num_task_tokens, :] # [B, L, NUM_TASK, D]
        actions_hidden_states = actions_hidden_states[:, :, self.num_task_tokens:, :] # [B, L, NUM_ACTIONS, D]
        
        if self.use_history_version:
            # actions_hidden_states_history: [B, H, L, NUM_TASK+NUM_ACTIONS, D]; slice token dim
            task_hidden_states_history = actions_hidden_states_history[:, :, :, :self.num_task_tokens, :]   # [B, H, L, NUM_TASK, D]
            actions_hidden_states_history = actions_hidden_states_history[:, :, :, self.num_task_tokens:, :]   # [B, H, L, NUM_ACTIONS, D]
        else:
            task_hidden_states_history = None
            actions_hidden_states_history = None
        ...
        if self.use_aug_mode_condition:
            aug_mode = aug_mode.to(device)
            aug_mode_features = self.aug_mode_proj(aug_mode)
            aug_mode_features = aug_mode_features.unsqueeze(dim=1)
        else:
            aug_mode_features = None
        ...
        action = self.model(
            ...
            h_t_history=task_hidden_states_history,
            h_a_history=actions_hidden_states_history,
            actions_history=actions_history,
            aug_mode_features=aug_mode_features,
            )
...
class MLPResNet(nn.Module):
    ...
    def forward(self,
        ...
        for i, block in enumerate(self.mlp_resnet_blocks):
            if self.use_history_version:
                # h_t_history/h_a_history: [B, H, L, N, D]; select layer i+1 -> [B, H, N, D]
                x = block(x, h_t=h_t[:, i+1, :], h_a=h_a[:, i+1, :], p=p,
                         h_t_history=h_t_history[:, :, i+1, :, :], h_a_history=h_a_history[:, :, i+1, :, :],
                         actions_history=actions_history)
            else:
                x = block(x, h_t = h_t[:,i+1,:], h_a = h_a[:,i+1,:], p=p, aug_mode_features=aug_mode_features)  # shape: (batch_size, hidden_dim)
...
class MLPResNetBlock_Pro(nn.Module):
    ...
    def forward(self, x, h_a=None, h_t=None, p=None, aug_mode_features=None):
        """
        h_a: adapter tokens
        h_t: task tokens
        ...
        if aug_mode_features is not None:
            h_adapter = torch.cat((h_a, p, aug_mode_features), dim=1)
        else:
...
class MLPResNetBlock_Pro_History(nn.Module):
    """History variant of MLP ResNet Block: extends Pro with KV and multi-head attention over h_t_history, h_a_history, actions_history, and an extra g_history scaling the history attention scores."""

    def __init__(self, dim, num_heads=8, action_dim=None, num_history_frames=None):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.head_dim = dim // num_heads
        ...
        self.num_history_frames = num_history_frames  # optional; when None, inferred from input
        ...
        # History modalities: separate K, V for each
        self.k_h_t_history = nn.Linear(dim, dim)
        self.v_h_t_history = nn.Linear(dim, dim)
        self.k_h_a_history = nn.Linear(dim, dim)
        self.v_h_a_history = nn.Linear(dim, dim)
        self.actions_history_proj = nn.Linear(self.action_dim, dim)
        self.k_actions_history = nn.Linear(dim, dim)
        self.v_actions_history = nn.Linear(dim, dim)

        self.o_proj = nn.Linear(dim, dim)

        self.gating_factor = nn.Parameter(torch.zeros(1))
        self.gating_factor_history = nn.Parameter(torch.zeros(1))  # scales history attention scores
    ...
    def forward(self, x, h_a=None, h_t=None, p=None, h_t_history=None, h_a_history=None, actions_history=None):
        g = self.gating_factor
        ratio_g = torch.tanh(g)
        g_hist = self.gating_factor_history
        ratio_g_history = torch.tanh(g_hist)
        ...
        # History: h_t_history/h_a_history are (B, H, N, D) from call chain; flatten to (B, H*N, D) for KV
        def reshape_heads(t, B, L):
            return t.view(B, L, self.num_heads, self.head_dim).transpose(1, 2)

        k_hist_list, v_hist_list = [], []
        if h_t_history is not None:
            # h_t_history: (B, H, N_t, D) -> (B, H*N_t, D)
            B_h, H_h, N_t, D_h = h_t_history.shape
            h_t_flat = h_t_history.reshape(B_h, H_h * N_t, D_h)
            k_ht = reshape_heads(self.k_h_t_history(h_t_flat), B_h, H_h * N_t)
            v_ht = reshape_heads(self.v_h_t_history(h_t_flat), B_h, H_h * N_t)
            k_hist_list.append(k_ht)
            v_hist_list.append(v_ht)
        if h_a_history is not None:
            # h_a_history: (B, H, N_a, D) -> (B, H*N_a, D)
            B_h, H_h, N_a, D_h = h_a_history.shape
            h_a_flat = h_a_history.reshape(B_h, H_h * N_a, D_h)
            k_ha = reshape_heads(self.k_h_a_history(h_a_flat), B_h, H_h * N_a)
            v_ha = reshape_heads(self.v_h_a_history(h_a_flat), B_h, H_h * N_a)
            k_hist_list.append(k_ha)
            v_hist_list.append(v_ha)
        if actions_history is not None:
            # actions_history: (B, H, action_dim) -> (B, H, D)
            a_proj = self.actions_history_proj(actions_history)
            L_act = a_proj.size(1)
            k_act = reshape_heads(self.k_actions_history(a_proj), B, L_act)
            v_act = reshape_heads(self.v_actions_history(a_proj), B, L_act)
            k_hist_list.append(k_act)
            v_hist_list.append(v_act)
        ...
        # Attention scores: self, adapter, task (with g), then history (with g_history)
        attn_scores = [torch.matmul(q_1, k_tokens.transpose(-2, -1))]
        attn_scores.append(torch.matmul(q_1, k_adapter.transpose(-2, -1)))
        attn_scores.append(torch.matmul(q_1, k_task.transpose(-2, -1)) * ratio_g)

        v_list = [v_tokens, v_adapter, v_task]
        if k_hist_list and v_hist_list:
            k_hist = torch.cat(k_hist_list, dim=2)   # (B, num_heads, 3*H_hist, head_dim)
            v_hist = torch.cat(v_hist_list, dim=2)
            K_hist = k_hist.size(2)
            cos_hist, sin_hist = self.rope(seq_len=K_hist, device=x.device, dtype=x.dtype)
            _, k_hist = apply_rope(k_hist, k_hist, cos_hist, sin_hist)
            attn_scores_hist = torch.matmul(q_1, k_hist.transpose(-2, -1)) * ratio_g_history
            attn_scores.append(attn_scores_hist)
