...
def prepare_observation(obs, resize_size, flip=False, cfg: GenerateConfig = None):
    ...
    if flip:
        if cfg.flip_type == 'horizontal':
            ...
            img = np.flip(img, axis=(0, 1)).copy()  # horizontal + vertical flip
...
def run_episode(
    ...
    try:
        while t < max_steps + cfg.num_steps_wait:
            ...
            # View fusion during evaluation
            if cfg.enable_view_fusion:
                # Fusion strength in [0, 1], derived from gripper state
                alpha = float(observation["state"][-1].copy())  # Gripper position in [0, 1]

                # log_message(f"View fusion alpha: {alpha:.4f}", log_file)
                alpha = (alpha - GRIPPER_RANGE[0]) / (GRIPPER_RANGE[1] - GRIPPER_RANGE[0])  # Normalize to [0, 1]
                # Optionally invert fusion direction
                if cfg.view_fusion_invert:
                    alpha = 1.0 - alpha

                # Convert images to float32 for fusion
                full_image_float = observation["full_image"].astype(np.float32)
                wrist_image_float = observation["wrist_image"].astype(np.float32)

                # View fusion:
                #   - alpha ↑ : wrist view brighter, full view darker
                #   - alpha ↓ : full view brighter, wrist view darker
...
def eval_libero(cfg: GenerateConfig) -> float:
    ...
    rows = []
    for task_description, success_count in per_task_success_counter.items():
        rate = (success_count / cfg.num_trials_per_task) * 100
        rows.append((task_description, success_count, cfg.num_trials_per_task, rate))

    # Calculate column widths
    col1_w = max(len("Task"), max(len(r[0]) for r in rows), len("**Overall**"))
    col2_w = max(len("Successes"), max(len(f"{r[1]}/{r[2]}") for r in rows), len(f"{total_successes}/{total_episodes}"))
    col3_w = max(len("Success Rate"), max(len(f"{r[3]:.1f}%") for r in rows), len(f"{final_success_rate * 100:.1f}%"))

    header    = f"| {'Task':<{col1_w}} | {'Successes':>{col2_w}} | {'Success Rate':>{col3_w}} |"
    separator = f"| {'-' * col1_w} | {'-' * col2_w}:| {'-' * col3_w}:|"
    overall   = f"| {'**Overall**':<{col1_w}} | {f'{total_successes}/{total_episodes}':>{col2_w}} | {f'{final_success_rate * 100:.1f}%':>{col3_w}} |"

    print("\n" + header)
    print(separator)
    for task_description, success_count, total, rate in rows:
        print(f"| {task_description:<{col1_w}} | {f'{success_count}/{total}':>{col2_w}} | {f'{rate:.1f}%':>{col3_w}} |")
    print(separator)
    print(overall + "\n")
