...
def get_action_head(cfg: Any, llm_dim: int) -> Union[L1RegressionActionHead]:
    ...
    use_aug_mode_condition = getattr(cfg, "use_aug_mode_condition", False)
    if cfg.use_l1_regression:
        action_head = L1RegressionActionHead(
            ...
            use_aug_mode_condition=use_aug_mode_condition,
        )
...
def get_vla_action(
    ...
    history_images: Optional[List[Any]] = None,
    history_actions: Optional[Union[List[np.ndarray], np.ndarray]] = None,
    aug_mode: Optional[Union[int, np.ndarray]] = None,
) -> List[np.ndarray]:
    """
    ...
    Args:
        ...
        history_images: Optional list of (primary_image,) or (primary_image, wrist_image, ...) per history frame
        history_actions: Optional [H, action_dim] or list of H actions for history conditioning
    ...
    with torch.inference_mode():
        ...
        # History conditioning is disabled: ignore history_images and history_actions
        ...
        if action_head is None:
            ...
            aug_mode_tensor = None
            if aug_mode is not None:
                if isinstance(aug_mode, (int, np.integer)):
                    aug_mode_tensor = torch.tensor([int(aug_mode)], dtype=torch.long, device=DEVICE)
                else:
                    aug_mode_tensor = torch.as_tensor(aug_mode, dtype=torch.long, device=DEVICE)
                    if aug_mode_tensor.dim() == 0:
                        aug_mode_tensor = aug_mode_tensor.unsqueeze(0)
