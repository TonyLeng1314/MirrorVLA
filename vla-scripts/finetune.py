...
class FinetuneConfig:
    ...
    #  mirrorVLA related
    # ========================
    wandb_mode                 : str   = "online"  # Wandb mode: online / offline
    mask_cam                   : str   = "None"    # Mask camera type: None / wrist / third
    vertical_flip_ratio        : float = 0.0       # The ratio to apply vertical flip for image augmentation
    horizontal_flip_ratio      : float = 0.0       # The ratio to apply horizontal flip for image augmentation
    history_frames             : int   = 0         # The number of history frames to use for the input
    enable_four_quadrant_input : bool  = False     # Whether to enable four quadrant input
    use_aug_mode_condition     : bool  = False     # Whether to use aug_mode as additional condition in action head
    
    # ========================
    #  History related (disabled)
    # ========================
    use_history_hidden_states: bool = False        # Deprecated: history conditioning is disabled
...
def run_forward_pass(
    ...
    with torch.autocast("cuda", dtype=torch.bfloat16):
        ...
        if cfg.vertical_flip_ratio > 0.0 :
            if torch.rand(1).item() < cfg.vertical_flip_ratio:
                batch['pixel_values'] = torch.flip(batch['pixel_values'], dims=[2])
                
        if cfg.horizontal_flip_ratio > 0.0 :
            if torch.rand(1).item() < cfg.horizontal_flip_ratio:
    ...
    # Compute metrics for continuous action representations (L1 regression)
        ...
        for item in output.hidden_states[0:]:
            ...
            # num_patches is num_vision_patches * num_image_in_input which is 256*2 if not specified
        ...
        multi_layer_hidden_states = torch.cat(multi_layer_hidden_states, dim = 1) # [B, L, NUM_TASK + NUM_ACTIONS, D]

        tmp_action_head: L1RegressionActionHead = action_head.module if isinstance(action_head, DDP) else action_head
        ...
        # Optional aug_mode conditioning (shape: [B], dtype: long)
        if getattr(cfg, "use_aug_mode_condition", False) and "aug_mode" in batch:
            # aug_mode: tensor of shape [B], long, same batch dimension as actions_hidden_states
            predict_kwargs["aug_mode"] = batch["aug_mode"].to(device_id)

        predicted_actions = tmp_action_head.predict_action(**predict_kwargs)
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if distributed_state.is_main_process:
        wandb.init(project=cfg.wandb_project, name=f"ft+{run_id}", mode=cfg.wandb_mode)
    ...
    if cfg.use_minivlm:
        ...
        config = AutoConfig.from_pretrained("pretrained_models/configs/config.json")
        vla:OpenVLAForActionPrediction = AutoModelForVision2Seq.from_config(config, torch_dtype=torch.bfloat16).to(device_id)  # Create a new model with configuration, the parameters are randomly initialized
    ...
    if cfg.use_l1_regression:
        action_head = init_module(
            ...
            device_id,
            {
                ...
                "use_aug_mode_condition": cfg.use_aug_mode_condition,
            },
            to_bf16=True,
        )
    ...
    batch_transform = RLDSBatchTransform(
        ...
        mask_cam=cfg.mask_cam,
        history_frames=0,
        history_compression_rate=1,
        enable_four_quadrant_input=cfg.enable_four_quadrant_input,
        )
